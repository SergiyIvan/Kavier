#!/usr/bin/env python3
"""Plot mean runtime vs. participation rate — five plots covering all experiment modes.

Each plot shows mean runtime (h) on the Y axis and participation rate (%) on the X axis.
...
5. exp-run-absolute-oracle-priorities.pdf
   Oracle blend with priorities enabled.
   6 lines: same structure as plot 4.

Usage:
    python plot-exp-run-absolute.py <results_dir>
...
from plot_utils import PERCENTAGES, SEEDS, load_and_merge, group_mean_runtime  # noqa: E402
...
def load_seeded(results_dir: Path, pattern: str) -> dict:
    ...
    for i, pct in enumerate(PERCENTAGES):
        for j, seed in enumerate(SEEDS):
            ...
            for group, mean_h in group_mean_runtime(df).items():
...
def plot_trad_priorities(results_dir: Path) -> None:
    ...
    fig.tight_layout()
    fig.savefig("exp-run-absolute-trad-priorities.pdf")
    plt.close(fig)
    print("Written: exp-run-absolute-trad-priorities.pdf")
...
def plot_random_blend(results_dir: Path) -> None:
    ...
    fig.tight_layout()
    fig.savefig("exp-run-absolute-random-blend.pdf")
    plt.close(fig)
    print("Written: exp-run-absolute-random-blend.pdf")
...
def plot_random_blend_priorities(results_dir: Path) -> None:
    ...
    fig.tight_layout()
    fig.savefig("exp-run-absolute-random-blend-priorities.pdf")
    plt.close(fig)
    print("Written: exp-run-absolute-random-blend-priorities.pdf")
...
def _plot_oracle(results_dir: Path, priorities: bool) -> None:
    ...
    out_name = "exp-run-absolute-oracle-priorities.pdf" if priorities else "exp-run-absolute-oracle.pdf"
