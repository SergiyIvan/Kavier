#!/usr/bin/env python3
"""Plot mean run time (± 1 std across seeds) vs. percentage of patched jobs.
...
Output:
    exp-run-absolute.pdf  (written to the current working directory)
"""
...
def compute_means(results_dir: Path) -> dict[str, np.ndarray]:
    """Return per-seed mean run time (h) arrays of shape (len(PERCENTAGES), len(SEEDS)).
...
def main() -> None:
    ...
    fig.tight_layout()
    fig.savefig("exp-run-absolute.pdf")
