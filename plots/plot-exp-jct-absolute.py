#!/usr/bin/env python3
"""Plot mean JCT vs. participation rate — five plots covering all experiment modes.

Each plot shows mean JCT (h) on the Y axis and participation rate (%) on the X axis.
...
5. exp-jct-absolute-oracle-priorities.pdf
   Oracle blend with priorities enabled.
   6 lines: same structure as plot 4.

Usage:
    python plot-exp-jct-absolute.py <results_dir>
...
def load_seeded(results_dir: Path, pattern: str) -> dict:
    ...
    for i, pct in enumerate(PERCENTAGES):
        for j, seed in enumerate(SEEDS):
            ...
            for group, mean_h in group_mean_jct(df).items():
...
def plot_trad_priorities(results_dir: Path) -> None:
    ...
    fig.tight_layout()
    fig.savefig("exp-jct-absolute-trad-priorities.pdf")
    plt.close(fig)
    print("Written: exp-jct-absolute-trad-priorities.pdf")
...
def plot_random_blend(results_dir: Path) -> None:
    ...
    fig.tight_layout()
    fig.savefig("exp-jct-absolute-random-blend.pdf")
    plt.close(fig)
    print("Written: exp-jct-absolute-random-blend.pdf")
...
def plot_random_blend_priorities(results_dir: Path) -> None:
    ...
    fig.tight_layout()
    fig.savefig("exp-jct-absolute-random-blend-priorities.pdf")
    plt.close(fig)
    print("Written: exp-jct-absolute-random-blend-priorities.pdf")
...
def _plot_oracle(results_dir: Path, priorities: bool) -> None:
    ...
    out_name = "exp-jct-absolute-oracle-priorities.pdf" if priorities else "exp-jct-absolute-oracle.pdf"
    ...
    for variant in ORACLE_VARIANTS:
        ...
        # Build series with variant tag appended to each label
        series = [
